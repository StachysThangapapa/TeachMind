...
from backend.services.embedding_service import get_default_embedding_provider  # noqa: E402
...
def seed() -> None:
    ...
    try:
        ...
        for skill_data in SAMPLE_SKILLS:
            ...
            if existing:
                print(f"  [Skip] {skill_data.skill_id} ({skill_data.name}) — already exists")
                continue
            service.store_skill(skill_data)
            print(f"  [Stored] {skill_data.skill_id} ({skill_data.name}) — stored with embedding")
