...
def main():
    ...
    try:
        ...
        print("✓ Cohere structured output extracted successfully:")
    ...
    try:
        ...
        print("✓ POST /skills stored skill in PostgreSQL + pgvector successfully!")
        results["POST /skills"] = "PASS"
    except urllib.error.HTTPError as e:
        if e.code == 409:
            print(f"Notice: Skill {skill_id} already exists in DB (from prior run). Continuing...")
            results["POST /skills"] = "PASS"
        else:
            print(f"✗ POST /skills failed: {e}")
    ...
    try:
        ...
        print(f"✓ Retrieved {len(skills_list)} skills from database. Stored skill is present!")
        results["My Skills retrieval"] = "PASS"
    except Exception as e:
        print(f"✗ GET /skills failed: {e}")
    ...
    try:
        ...
        print(f"✓ Top match: {top_hit['name']} (ID: {top_hit['skill_id']})")
        print(f"   Similarity: {top_hit['similarity']}")
        assert top_hit["similarity"] > 0.5, f"Similarity {top_hit['similarity']} too low"
        assert top_hit["skill"]["skill_id"] == top_hit["skill_id"], "Nested skill JSON mismatch"
        print("✓ Complete canonical Skill JSON returned in response.results[0].skill")
    ...
    try:
        ...
        print("✓ Skill successfully marked as verified!")
    ...
    try:
        ...
        print(f"✓ Correction applied! Version bumped to v{correct_res['version']}")
    ...
    try:
        ...
        print(f"✓ Version history preserved successfully! Versions found: {v_nums}")
    ...
    try:
        ...
        print("✓ Stored skill verified directly in PostgreSQL!")
